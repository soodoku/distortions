...
scores <- bind_rows(
  ...
  map(c("gender", "educ", "income", "triple"), \(dimension) {
    read.csv(sprintf("tabs_clean/03_dom_%s_by_group_issue.csv", dimension)) |>
...
write.csv(paired_comparison, "tabs_clean/09_paired_response_sensitivity.csv", row.names = FALSE)
write.csv(frequency_comparison, "tabs_clean/09_frequency_comparison.csv", row.names = FALSE)
