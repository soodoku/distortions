...
run_order <- c("01_summary_dp_data_table_1.R",
...
compare_csv <- function(f_new, f_old, tol) {
    ...
    if (!identical(dim(a), dim(b)) || !identical(names(a), names(b))) {
        return(list(match = FALSE, n_mismatch = NA, max_diff = NA,
                    note = sprintf("shape %dx%d vs %dx%d",
    ...
    list(match = num_bad + txt_bad == 0, n_mismatch = num_bad + txt_bad,
         max_diff = if (length(diffs)) max(diffs) else 0, note = "")
}

committed <- list.files(file.path(repo, "tabs"), pattern = "\\.csv$")
manifest <- data.frame(file = committed, regenerated = NA, match = NA,
...
write.csv(data.frame(script = run_order, status = status),
          "tabs_clean/90_repro_script_status.csv", row.names = FALSE)
