...
anglo <- c("UK EU", "UK Health", "UK Monarchy", "UK General Election",
...
split_density_fig <- function(d, split, lab, file) {
    p <- ggplot(d, aes(value, fill = .data[[split]], color = .data[[split]],
                  group = .data[[split]])) +
...
poll_names <- read.csv("data/poll_indices.csv") |>
    distinct(dpnum, poll_name)

hp <- read.csv("tabs_clean/03_hom_pol_by_group_issue.csv") |>
    left_join(poll_names, by = c(poll_id = "dpnum"))

dom_pairs <- function(dim_name) {
    read.csv(sprintf("tabs_clean/03_dom_%s_by_group_issue.csv", dim_name)) |>
        rename(value = extgrp_grp)
}

sets <- list(
    polarization   = list(d = hp |> rename(value = polarex),
                          lab = expression(P["gj"]), dom = FALSE),
    homogenization = list(d = hp |> rename(value = homoex),
...
for (name in names(sets)) {
    ...
    d <- s$d |>
        filter(!is.na(value)) |>
        mutate(anglo = if_else(poll_name %in% anglo, "Anglo", "Other"),
    ...
    split_density_fig(d, "anglo", s$lab,
                      sprintf("figs_clean/density_anglo_%s.png", name))
    split_density_fig(d, "online", s$lab,
                      sprintf("figs_clean/density_online_f2f_%s.png", name))
    if (s$dom) {
        split_boxplot_fig(d, "anglo", s$lab,
